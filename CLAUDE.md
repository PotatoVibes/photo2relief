...
- **Per-backend depth convention (verified empirically in M2, RTX 3080):**
  ...
  - **`da3mono-large` (DA3MONO)** → native output is **direct depth, larger = farther**; adapter would use `convention="depth"` (flip via negate before normalize). **Not empirically verified — DA3 backend deferred to M2.5** (see decisions log). The `normalize_to_closest(raw, "depth")` path IS unit-tested with synthetic data.
...
- `depth_anything_3` (v0.1.1, only release) is **not integrated** — it pins `numpy<2` and drags in open3d/xformers/pycolmap/moviepy/gsplat, a hard conflict with our numpy-2 / trimesh / opencv stack (and a Windows install minefield). Deferred to **M2.5** per the SPEC risk table; the `DA3Backend` adapter exists and raises a clear `BackendUnavailableError` pointing users to the DA2 models. Wiring it later needs subprocess isolation (separate venv).
...
| **M2.5 DA3 backend (`da3mono-large`)** | ☐ deferred | Wire `depth_anything_3` via subprocess isolation (separate venv, numpy<2). Then set `da3mono-large.available=True`, make it the GPU default, verify `convention="depth"` flip empirically, update this table. |
